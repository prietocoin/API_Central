...
            return res.json([ratesObject]); // Devuelve un array con el objeto
        ...
        // --------------------------------------------------------------------------
        // FORMATO 5: CRUCE DE TABLA (JSON fila-columna)
        case 'json_cruce':
            const crossData = utils.transformToCrossJoin(rawData);
            return res.json(crossData);
        // --------------------------------------------------------------------------
...
                detalle: "Formatos válidos: raw, json, json_ultima_fila, json_clave_valor, json_cruce." // <-- Lista actualizada
            });
    }
}
